...
const PreferencesInputSchema = z.object({
  ...
  preferred_ingredients: z.string().max(1000, "preferred_ingredients must be at most 1000 characters").optional().default(""),
...
const PreferencesPartialInputSchema = z.object({
  diet_type: z.string().min(1, "diet_type cannot be empty").max(50, "diet_type must be at most 50 characters").optional(),
...
export const PreferencesInputSchema = z.object({
  ...
  preferred_cuisines: z
    .string()
    .max(500, "preferred_cuisines must be at most 500 characters")
    .optional()
    .default(""),
  allergens: z
    .string()
    .max(500, "allergens must be at most 500 characters")
    .optional()
    .default(""),
  notes: z
    .string()
...
import {
  ...
  PreferencesPartialInputSchema
} from "../../../lib/validation/preferences.schemas";
import {
  getUserPreferences,
...
export const PATCH: APIRoute = async ({ request, locals }) => {
  ...
  try {
    const updatedPreferences = await patchUserPreferences(
      user.id,
      validatedData,
      locals.supabase
    );
